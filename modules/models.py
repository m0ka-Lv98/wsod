...
class Faster_RCNN(nn.Module):
    def __init__(self, num_classes, block, layers):
        ...
        self.icr2 = ICR(1024)
        self.icr3 = ICR(1024)
        self.center = nn.Sequential(ASPP([1,2,3]),
                                nn.Flatten(),
                                nn.Linear(out*(14), 512))
        self.aux = nn.Sequential(ASPP([1,2,3]),
                                nn.Flatten(),
                                nn.Linear(out*(14), 4))
        ...
        #self.freeze_bn()
        self.MIL = nn.Sequential(self.conv1,
                                 self.bn1,
                                 self.relu,
                                 self.maxpool,
                                 self.layer1,
        ...
        self.OTHERS = nn.Sequential(self.RCNN_rpn,
                                    self.RCNN_c,
                                    self.RCNN_r)
    ...
    def mil(self):
        return self.MIL.parameters()
    def others(self):
        return self.OTHERS.parameters()

    def forward(self, image, labels,R=None,n=None):
        try:
            bs = labels.shape[0]
        except:
            bs = 1
...
class sFaster_RCNN(nn.Module):
    ...
    def forward(self, image, annot=None):
        ...
        v = self.feature_vector(f)
        if self.training:
            rpn_closs,rpn_rloss = self.rpn_loss(rpn_c, rpn_r, anchors, annot)
        ...
        r = r.view(bs,num,-1)
        if self.training:
            closs,rloss = self.f_loss(c, r, rois, annot)
            closs = closs.mean()
            rloss = rloss.mean()
            rpn_closs = rpn_closs.mean()
            rpn_rloss = rpn_rloss.mean()
            print(f'f_c={closs:.4f},f_r={rloss:.4f},rpn_c={rpn_closs:.4f},rpn_r={rpn_rloss:.4f}')
            loss = closs+rloss+rpn_closs+rpn_rloss
            return x, loss.unsqueeze(0),(closs+rloss).unsqueeze(0),(rpn_closs+rpn_rloss).unsqueeze(0)
...
def Faster_RCNN18(num_classes, pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = sFaster_RCNN(num_classes, BasicBlock, [2, 2, 2, 2], **kwargs)
    dl_root = "/data/unagi0/masaoka/resnet_model_zoo/"
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18'], model_dir=dl_root), strict=False)
    return model
...
def Faster_RCNN50(num_classes, pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = sFaster_RCNN(num_classes, Bottleneck, [3, 4, 6, 3], **kwargs)
    dl_root = "/data/unagi0/masaoka/resnet_model_zoo/"
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50'], model_dir=dl_root), strict=False)
    return model
