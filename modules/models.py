...
class ResNet_rpn(nn.Module):
    ...
    def forward(self, image, labels, rois,dum):
        if self.training:
            ...
            rois,rpn_c,rpn_r,anchors = self.RCNN_rpn(features, np.array([512,512]))
            s = time.time()
            #print(f'rpn {s-e}')
            num = rois.shape[1]
            f = self.multi_scale_roi_pool(features[0],features[1],features[2], rois)
            ...
            rpn_closs,rpn_rloss = self.rpn_loss(rpn_c, rpn_r, anchors, gt_list,SOFT=True)
            s = time.time()
            ...
            anchors = self.anchors(img_batch)
            #print(gt_list)
            c_loss,r_loss = self.focalLoss(classification, regression, anchors, gt_list,SOFT=True)
            s = time.time()
