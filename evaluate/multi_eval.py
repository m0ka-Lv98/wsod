...
def evaluate_coco(dataset, model, val,threshold=0.05):
    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            rois = p_box.cuda().float().unsqueeze(0)
            pro = p_box.shape[0]
        ...
        # write output
        json.dump(results, open(f'/data/unagi0/masaoka/wsod/result_bbox/midn{val}.json', 'w'), indent=4)

        # load results in COCO evaluation tool
        coco_true = dataset.coco
        coco_pred = coco_true.loadRes(f'/data/unagi0/masaoka/wsod/result_bbox/midn{val}.json')
...
if __name__=='__main__':
    ...
    retinanet = multi_midn()
    retinanet.cuda()
    retinanet.load_state_dict(torch.load(f'/data/unagi0/masaoka/wsod/model/oicr/multi_midnanchornosched1e-05_{val}_6.pt'))
    evaluate_coco(dataset, retinanet, val,threshold=0.05)

#multi_midnanchornosched1e-05_{val}_6.pt (fpn+midn rms)  midn