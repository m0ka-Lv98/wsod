    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            if torch.cuda.is_available():
                scores, labels, boxes = model(data['img'].cuda().float().unsqueeze(dim=0),0,0,0)
            else:
        ...
        # write output
        json.dump(results, open(f'/data/unagi0/masaoka/wsod/result_bbox/midn_retina{val}.json', 'w'), indent=4)

        # load results in COCO evaluation tool
        coco_true = dataset.coco
        coco_pred = coco_true.loadRes(f'/data/unagi0/masaoka/wsod/result_bbox/midn_retina{val}.json')
...
if __name__=='__main__':
    ...
    retinanet = resnet_midn(num_classes=3, pretrained=True)
    retinanet.cuda()
    retinanet.load_state_dict(torch.load(f'/data/unagi0/masaoka/wsod/model/oicr/SLV_Retinaanchormidn1e-05_0_6.pt'))
