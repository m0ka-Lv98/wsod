...
from rpn._proposal_layer_fpn import _ProposalLayer_FPN
from rpn._anchor_target_layer_fpn import _AnchorTargetLayer_FPN
from rpn.net_utils import _smooth_l1_loss
...
class _RPN_FPN(nn.Module):
    ...
    def forward(self, rpn_feature_maps, im_info, gt_boxes=None,rpn_cls_score_alls=None,rpn_bbox_pred_alls=None,rpn_shapes=None):
        if not isinstance(gt_boxes,torch.Tensor):  
            ...
            for i in range(n_feat_maps):
                ...
                rpn_cls_probs.append(torch.softmax(rpn_cls_score.permute(0, 2, 3, 1).contiguous().view(batch_size, -1, 2),-1))
            ...
            return rois,rpn_cls_score_alls,rpn_bbox_pred_alls,rpn_shapes
        else:
            self.rpn_loss_cls = 0
            self.rpn_loss_box = 0
            ...
            if self.training:
                ...
                rpn_data = self.RPN_anchor_target((rpn_cls_score_alls.data, gt_boxes, im_info, rpn_shapes))

                # compute classification loss
                rpn_label = rpn_data[0].view(self.bs, -1)
                ...
                rpn_bbox_inside_weights = rpn_bbox_inside_weights.unsqueeze(2) \
                    .expand(self.bs, rpn_bbox_inside_weights.size(1), 4)
                rpn_bbox_outside_weights = rpn_bbox_outside_weights.unsqueeze(2) \
                    .expand(self.bs, rpn_bbox_outside_weights.size(1), 4)
