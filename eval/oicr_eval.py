...
def evaluate_coco(dataset, model, val,threshold=0.05):
    ...
    with torch.no_grad():
        ...
        # write output
        json.dump(results, open(f'/data/unagi0/masaoka/wsod/result_bbox/oicr{val}.json', 'w'), indent=4)

        # load results in COCO evaluation tool
        coco_true = dataset.coco
        coco_pred = coco_true.loadRes(f'/data/unagi0/masaoka/wsod/result_bbox/oicr{val}.json')
...
if __name__=='__main__':
    ...
    retinanet = OICR()
    retinanet.cuda()
    retinanet.load_state_dict(torch.load(f'/data/unagi0/masaoka/wsod/model/oicr/OICRanchor1e-05_{val}_6.pt'))
