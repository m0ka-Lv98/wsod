...
import dataset.fast_dloader as fd
import dataset.faster_dloader as frd
import subprocess
...
parser.add_argument('-m', '--model', type=str, default="Hybrid")
...
def main():
    ...
    box = make_anchor()
    dl_t, _, _, _, _ = fd.make_data(batchsize=args.batchsize,iteration=args.iteration,val=args.val)
    for epoch in range(args.resume,args.epochs):
        if epoch == 1:
            del dl_t
            dl_t, _, _, _, _ = frd.make_data(batchsize=args.batchsize,iteration=args.iteration,val=args.val)
        for i, data in enumerate(dl_t,1):
            ...
            if epoch==0:
                ...
                output, loss,m,l1,l2,l3,lf,lr = oicr(data["img"].cuda().float(), labels, rois)
            else:
